...
@implementation MKRGSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_rg_wifiSecurity)security {
    switch (security) {
        case mk_rg_wifiSecurity_personal:
            return @"00";
        case mk_rg_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_rg_eapType)eapType {
    switch (eapType) {
        case mk_rg_eapType_peap_mschapv2:
            return @"00";
        case mk_rg_eapType_ttls_mschapv2:
            return @"01";
        case mk_rg_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_rg_connectMode)mode {
    switch (mode) {
        case mk_rg_connectMode_TCP:
            return @"00";
        case mk_rg_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_rg_connectMode_CACertificate:
            return @"02";
        case mk_rg_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_rg_mqttServerQosMode)mode {
    switch (mode) {
        case mk_rg_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_rg_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_rg_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
