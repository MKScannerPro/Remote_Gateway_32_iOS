...
@implementation MKRGUserCredentialsViewModel
@end

@interface MKRGUserCredentialsView ()
...
@implementation MKRGUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKRGUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rg_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate rg_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rg_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate rg_mqtt_userCredentials_passwordChanged:text];
            }
        };
