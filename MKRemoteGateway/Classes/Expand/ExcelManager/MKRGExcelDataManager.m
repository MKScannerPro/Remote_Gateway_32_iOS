...
@implementation MKRGExcelDataManager

+ (void)exportAppExcel:(id <MKRGExcelAppProtocol>)protocol
              sucBlock:(void(^)(void))sucBlock
           failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRGExcelAppProtocol)]) {
...
+ (void)exportDeviceExcel:(id <MKRGExcelDeviceProtocol>)protocol
                 sucBlock:(void(^)(void))sucBlock
              failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRGExcelDeviceProtocol)]) {
        [self operationFailedBlockWithMsg:@"Params Error" block:failedBlock];
        return;
    }
...
+ (BOOL)checkExcelAppProtocol:(id <MKRGExcelAppProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRGExcelAppProtocol)]) {
        return NO;
    }
...
+ (BOOL)checkExcelDeviceProtocol:(id <MKRGExcelDeviceProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKRGExcelDeviceProtocol)]) {
        return NO;
    }
    if (!ValidStr(protocol.host) || protocol.host.length > 64 || ![protocol.host isAsciiString]) {
