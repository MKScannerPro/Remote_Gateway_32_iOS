...
@implementation MKRGExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            MKRGExcelSheet *sheet = [[MKRGExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [MKRGExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
...
/**
 ...
 @return 工作表
 */
-(MKRGExcelSheet *)getSheetWithSheetName:(NSString *)sheetName
{
    MKRGExcelSheet *sheet = nil;
...
-(NSDictionary *)getSharedStringsWithPath:(NSString *)path
{
    ...
    if(xmlData && xmlData.length > 0)
    {
        ...
        xmlDic = [MKRGXMLReader dictionaryForXMLData:xmlData error:&error];
    }
    return xmlDic;
}
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[MKRGExcelCell getNumberFromStr:fileName] integerValue];
                
                NSMutableDictionary *sheetDic = [[NSMutableDictionary alloc]init];
                
                NSDictionary *xmlDic = [MKRGXMLReader dictionaryForXMLData:xmlData error:&error];
...
-(NSArray *)getSheedNameInfoArrayWithPath:(NSString *)path
{
    ...
    NSDictionary *sheetNameInfoDic = [MKRGXMLReader dictionaryForXMLData:xmlData error:&error];
