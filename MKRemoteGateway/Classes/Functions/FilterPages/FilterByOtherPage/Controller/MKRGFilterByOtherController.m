...
@interface MKRGFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKRGFilterEditSectionHeaderViewDelegate,
MKRGFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKRGFilterByOtherModel *dataModel;

@end

@implementation MKRGFilterByOtherController

- (void)dealloc {
    NSLog(@"MKRGFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        MKRGFilterEditSectionHeaderViewModel *model = [[MKRGFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRGFilterByRawDataCell *cell = [MKRGFilterByRawDataCell initCellWithTableView:tableView];
...
- (void)mk_rg_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKRGFilterByRawDataCellModel *cellModel = [[MKRGFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKRGFilterByRawDataCellDelegate
/// 输入框内容发生改变
...
- (void)rg_rawFilterDataChanged:(mk_rg_filterRawAdvDataTextType)textType
    ...
    MKRGFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_rg_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_rg_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_rg_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_rg_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKRGFilterByRawDataCellModel *cellModel = self.section1List[i];
        ...
        MKRGFilterRawAdvDataModel *model = [[MKRGFilterRawAdvDataModel alloc] init];
...
- (mk_rg_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_rg_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_rg_filterByOther_AB;
        }
        return mk_rg_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_rg_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_rg_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_rg_filterByOther_AOrBOrC;
        }
    }
    return mk_rg_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByOtherController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
