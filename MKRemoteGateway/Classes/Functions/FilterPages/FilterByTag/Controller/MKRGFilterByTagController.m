...
@interface MKRGFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGFilterByTagModel *dataModel;

@end

@implementation MKRGFilterByTagController

- (void)dealloc {
    NSLog(@"MKRGFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByTagController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByTagModel alloc] init];
    }
    return _dataModel;
}
