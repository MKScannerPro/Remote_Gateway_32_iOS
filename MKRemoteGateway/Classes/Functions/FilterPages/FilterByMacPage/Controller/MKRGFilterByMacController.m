...
@interface MKRGFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGFilterByMacModel *dataModel;

@end

@implementation MKRGFilterByMacController

- (void)dealloc {
    NSLog(@"MKRGFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByMacController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByMacModel alloc] init];
    }
    return _dataModel;
}
