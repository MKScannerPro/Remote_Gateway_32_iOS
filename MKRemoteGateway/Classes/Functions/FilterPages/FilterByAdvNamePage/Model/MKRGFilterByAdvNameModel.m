...
@interface MKRGFilterByAdvNameModel ()
...
@implementation MKRGFilterByAdvNameModel
...
- (BOOL)readFilterByName {
    ...
    [MKRGMQTTInterface rg_readFilterByADVNameWithMacAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByName:(NSArray <NSString *>*)nameList {
    __block BOOL success = NO;
    [MKRGMQTTInterface rg_configFilterByADVName:nameList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
