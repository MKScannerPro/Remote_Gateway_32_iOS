...
@interface MKRGFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGFilterByPirModel *dataModel;

@end

@implementation MKRGFilterByPirController

- (void)dealloc {
    NSLog(@"MKRGFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByPirController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByPirModel alloc] init];
    }
    return _dataModel;
}
