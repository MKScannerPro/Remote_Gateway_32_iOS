...
@interface MKRGFilterByButtonModel ()
...
@implementation MKRGFilterByButtonModel
...
- (BOOL)configFilterByBXPButton {
    __block BOOL success = NO;
    [MKRGMQTTInterface rg_configFilterBXPButton:self macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
