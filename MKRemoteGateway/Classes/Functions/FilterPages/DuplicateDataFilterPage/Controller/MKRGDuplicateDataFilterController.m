//
//  MKRGDuplicateDataFilterController.m
...
@interface MKRGDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGDuplicateDataFilterModel *dataModel;

@end

@implementation MKRGDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKRGDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGDuplicateDataFilterController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
