//
//  MKRGDuplicateDataFilterModel.m
...
@interface MKRGDuplicateDataFilterModel ()
...
@implementation MKRGDuplicateDataFilterModel
...
- (BOOL)readDuplicateDataFilter {
    ...
    [MKRGMQTTInterface rg_readDuplicateDataFilterDatasWithMacAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKRGMQTTInterface rg_configDuplicateDataFilter:self.rule period:[self.time longLongValue] macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
