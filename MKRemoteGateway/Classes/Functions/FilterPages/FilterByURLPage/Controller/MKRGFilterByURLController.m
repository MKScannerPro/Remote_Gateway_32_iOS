...
@interface MKRGFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGFilterByURLModel *dataModel;

@end

@implementation MKRGFilterByURLController

- (void)dealloc {
    NSLog(@"MKRGFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByURLController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByURLModel alloc] init];
    }
    return _dataModel;
}
