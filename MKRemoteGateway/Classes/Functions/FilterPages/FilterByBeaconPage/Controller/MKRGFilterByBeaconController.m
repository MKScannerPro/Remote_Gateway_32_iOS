//
//  MKRGFilterByBeaconController.m
...
@property (nonatomic, strong)MKRGFilterByBeaconModel *dataModel;

@end

@implementation MKRGFilterByBeaconController

- (void)dealloc {
    NSLog(@"MKRGFilterByBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByBeaconController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByBeaconModel alloc] initWithPageType:self.pageType];
    }
    return _dataModel;
}
