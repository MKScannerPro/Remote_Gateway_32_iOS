...
@interface MKRGFilterByUIDController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGFilterByUIDModel *dataModel;

@end

@implementation MKRGFilterByUIDController

- (void)dealloc {
    NSLog(@"MKRGFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGFilterByUIDController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
