...
@implementation MKRGMQTTSSLForAppViewModel
@end

@interface MKRGMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKRGMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate rg_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate rg_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate rg_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate rg_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRGMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 2) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForAppView", @"rg_switchSelectedIcon.png") : LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForAppView", @"rg_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForAppView", @"rg_switchUnselectedIcon.png") forState:UIControlStateNormal];
