//
//  MKRGServerConfigAppFooterView.m
...
@implementation MKRGServerConfigAppFooterViewModel
@end
...
@implementation MKRGServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate rg_mqtt_serverForApp_qosChanged:qos];
    }
}
...
- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
#pragma mark - MKRGMQTTSSLForAppViewDelegate
- (void)rg_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)rg_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate rg_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)rg_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForApp_fileButtonPressed:0];
    }
}

/// 用户点击选择了P12证书按钮
- (void)rg_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForApp_bottomButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForApp_bottomButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForApp_bottomButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKRGServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKRGMQTTSSLForAppViewModel *sslModel = [[MKRGMQTTSSLForAppViewModel alloc] init];
...
- (MKRGMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKRGMQTTSSLForAppView alloc] init];
