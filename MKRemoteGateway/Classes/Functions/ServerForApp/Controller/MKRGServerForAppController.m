...
@interface MKRGServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRGServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKRGServerForAppModel *dataModel;

@property (nonatomic, strong)MKRGServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKRGServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKRGServerForAppController

- (void)dealloc {
    NSLog(@"MKRGServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKRGMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKRGMQTTDataManager shared] connect];
}
...
#pragma mark - MKRGServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)rg_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)rg_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)rg_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)rg_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)rg_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)rg_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)rg_selectedServerParams:(NSString *)fileName {
    [MKRGExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKRGServerForAppModel *model = [MKRGServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKRGMQTTDataManager shared].state == MKRGMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKRGMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_rg_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKRGExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKRGMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKRGMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGServerForAppController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRGServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRGServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKRGServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRGServerConfigAppFooterViewModel alloc] init];
    }
