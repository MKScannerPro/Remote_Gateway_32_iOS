...
@implementation MKRGScanPageCell

+ (MKRGScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKRGScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRGScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKRGScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRGScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKRGScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGScanPageModel.class]) {
        return;
    }
