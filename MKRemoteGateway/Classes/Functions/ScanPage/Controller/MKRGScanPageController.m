...
static NSString *const localPasswordKey = @"mk_rg_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKRGScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_rg_centralManagerScanDelegate>
...
@implementation MKRGScanPageController

- (void)dealloc {
    NSLog(@"MKRGScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKRGCentralManager shared] stopScan];
    [MKRGCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKRGCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKRGCentralManager shared].centralStatus != mk_rg_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKRGScanPageCell *cell = [MKRGScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_rg_centralManagerScanDelegate

- (void)mk_rg_receiveDevice:(NSDictionary *)deviceModel {
    MKRGScanPageModel *dataModel = [MKRGScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKRGCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKRGScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKRGScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKRGCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKRGScanPageModel *)deviecModel {
    ...
    [[MKRGCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    MKRGDeviceParamsListController *vc = [[MKRGDeviceParamsListController alloc] init];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKRemoteGateway", @"MKRGScanPageController", @"rg_scanRefresh.png");
    }
    return _refreshIcon;
}
