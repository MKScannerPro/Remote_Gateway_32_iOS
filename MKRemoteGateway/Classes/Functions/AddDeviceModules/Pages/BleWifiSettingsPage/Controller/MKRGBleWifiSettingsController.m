//
//  MKRGBleWifiSettingsController.m
...
#include "MKRGBleWifiSettingsCertCell.h"
...
@interface MKRGBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKRGBleWifiSettingsCertCellDelegate,
...
@property (nonatomic, strong)MKRGBleWifiSettingsModel *dataModel;

@end

@implementation MKRGBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKRGBleWifiSettingsController销毁");
}
...
#pragma mark - MKRGBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKRGBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKRGBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKRGBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKRGDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKRGDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKRGBleWifiSettingsCertCell *cell = [MKRGBleWifiSettingsCertCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKRGBleWifiSettingsCertCellModel *cellModel = [[MKRGBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGBleWifiSettingsController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
