//
//  MKRGBleNTPTimezoneController.m
...
@interface MKRGBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKRGBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKRGBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGBleNTPTimezoneController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
