//
//  MKRGServerForDeviceController.m
...
@interface MKRGServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRGServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKRGImportServerControllerDelegate>
...
@property (nonatomic, strong)MKRGServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKRGServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKRGServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKRGServerForDeviceController

- (void)dealloc {
    NSLog(@"MKRGServerForDeviceController销毁");
}
...
#pragma mark - MKRGServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)rg_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)rg_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)rg_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)rg_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)rg_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKRGImportServerController *vc = [[MKRGImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKRGImportServerControllerDelegate
- (void)rg_selectedServerParams:(NSString *)fileName {
    [MKRGExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKRGServerForDeviceModel *model = [MKRGServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKRGDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKRGDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKRGDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKRGDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKRGDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKRGDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKRGDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKRGDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKRGExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGServerForDeviceController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRGServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRGServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKRGServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRGServerConfigDeviceFooterViewModel alloc] init];
    }
