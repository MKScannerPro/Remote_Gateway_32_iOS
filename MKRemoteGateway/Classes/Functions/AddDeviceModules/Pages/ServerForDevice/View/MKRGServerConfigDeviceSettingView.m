//
//  MKRGServerConfigDeviceSettingView.m
...
@interface MKRGServerConfigDeviceSettingView ()
...
@implementation MKRGServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
