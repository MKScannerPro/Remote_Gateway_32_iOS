//
//  MKRGServerConfigDeviceFooterView.m
...
@implementation MKRGServerConfigDeviceFooterViewModel
@end
...
@interface MKRGServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKRGMQTTSSLForDeviceViewDelegate,
MKRGServerConfigDeviceSettingViewDelegate,
MKRGMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKRGMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKRGMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKRGServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKRGServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate rg_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}
...
- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKRGMQTTSSLForDeviceViewDelegate
- (void)rg_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)rg_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate rg_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)rg_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)rg_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)rg_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKRGServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)rg_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate rg_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKRGMQTTLWTForDeviceViewDelegate
- (void)rg_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)rg_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate rg_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)rg_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate rg_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)rg_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)rg_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate rg_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKRGServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKRGMQTTSSLForDeviceViewModel *sslModel = [[MKRGMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKRGMQTTLWTForDeviceViewModel *lwtModel = [[MKRGMQTTLWTForDeviceViewModel alloc] init];
...
- (MKRGMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKRGMQTTSSLForDeviceView alloc] init];
...
- (MKRGMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKRGMQTTLWTForDeviceView alloc] init];
...
- (MKRGServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKRGServerConfigDeviceSettingView alloc] init];
