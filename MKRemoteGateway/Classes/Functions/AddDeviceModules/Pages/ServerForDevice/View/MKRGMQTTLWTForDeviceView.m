...
@implementation MKRGMQTTLWTForDeviceViewModel
@end

@interface MKRGMQTTLWTForDeviceView ()
...
@implementation MKRGMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchSelectedIcon.png") : LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(rg_lwt_statusChanged:)]) {
        [self.delegate rg_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchSelectedIcon.png") : LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(rg_lwt_retainChanged:)]) {
        [self.delegate rg_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(rg_lwt_qosChanged:)]) {
        [self.delegate rg_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRGMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMQTTLWTForDeviceView", @"rg_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rg_lwt_topicChanged:)]) {
                [self.delegate rg_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(rg_lwt_payloadChanged:)]) {
                [self.delegate rg_lwt_payloadChanged:text];
            }
        };
