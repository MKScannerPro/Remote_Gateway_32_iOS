...
@implementation MKRGMQTTSSLForDeviceViewModel
@end

@interface MKRGMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKRGMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate rg_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate rg_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate rg_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate rg_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(rg_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate rg_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKRGMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForDeviceView", @"rg_switchSelectedIcon.png") : LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForDeviceView", @"rg_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMQTTSSLForDeviceView", @"rg_switchUnselectedIcon.png") forState:UIControlStateNormal];
