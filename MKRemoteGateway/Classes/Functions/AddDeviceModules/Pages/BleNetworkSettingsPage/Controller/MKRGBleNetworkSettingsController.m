//
//  MKRGBleNetworkSettingsController.m
...
@interface MKRGBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGBleNetworkSettingsModel *dataModel;

@end

@implementation MKRGBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKRGBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGBleNetworkSettingsController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
