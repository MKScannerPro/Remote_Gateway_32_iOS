...
@interface MKRGBleScannerFilterModel ()
...
@implementation MKRGBleScannerFilterModel
...
- (BOOL)readFilterRssi {
    ...
    [MKRGInterface rg_readRssiFilterValueWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterRssi {
    __block BOOL success = NO;
    [MKRGInterface rg_configRssiFilterValue:self.rssi sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacAddress {
    __block BOOL success = NO;
    [MKRGInterface rg_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacAddress {
    ...
    [MKRGInterface rg_configFilterMACAddressList:macList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)readAdvName {
    __block BOOL success = NO;
    [MKRGInterface rg_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvName {
    ...
    [MKRGInterface rg_configFilterAdvNameList:nameList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)configRelationShip {
    __block BOOL success = NO;
    [MKRGInterface rg_configFilterRelationship:7
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
