//
//  MKRGManageBleDevicesSearchButton.m
...
@implementation MKRGManageBleDevicesSearchButtonModel
@end

@interface MKRGManageBleDevicesSearchButton ()
...
@implementation MKRGManageBleDevicesSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(rg_scanSearchButtonClearMethod)]) {
        [self.delegate rg_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(rg_scanSearchButtonMethod)]) {
        [self.delegate rg_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKRGManageBleDevicesSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGManageBleDevicesSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKRemoteGateway", @"MKRGManageBleDevicesSearchButton", @"rg_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGManageBleDevicesSearchButton", @"rg_clearButtonIcon.png") forState:UIControlStateNormal];
