...
@implementation MKRGManageBleDevicesCellModel
@end

@interface MKRGManageBleDevicesCell ()
...
@implementation MKRGManageBleDevicesCell

+ (MKRGManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKRGManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRGManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKRGManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRGManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate rg_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRGManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGManageBleDevicesCellModel.class]) {
        return;
    }
