...
#import "MKRGButtonDFUModel.h"

@interface MKRGButtonDFUController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGButtonDFUModel *dataModel;
...
@implementation MKRGButtonDFUController

- (void)dealloc {
    NSLog(@"MKRGButtonDFUController销毁");
...
- (void)addNotifications {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuResult:)
                                                 name:MKRGReceiveBxpButtonDfuResultNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)gotoLastPage {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:NSClassFromString(@"MKRGManageBleDevicesController")]) {
            //如果是从Manage BLE devices页面进来的
            [self popToViewControllerWithClassName:@"MKRGManageBleDevicesController"];
            break;
        }
        if ([vc isKindOfClass:NSClassFromString(@"MKRGDeviceDataController")]) {
            //如果是从扫描页面进来的
...
- (MKRGButtonDFUModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGButtonDFUModel alloc] init];
