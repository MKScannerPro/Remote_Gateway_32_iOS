//
//  MKRGNormalConnectedController.m
...
@interface MKRGNormalConnectedController ()<UITableViewDelegate,
UITableViewDataSource,
MKRGNormalConnectedCellDelegate>
...
@property (nonatomic, strong)MKRGConnectedDeviceWriteAlertView *writeAlert;

@end

@implementation MKRGNormalConnectedController

- (void)dealloc {
    NSLog(@"MKRGNormalConnectedController销毁");
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKRGNormalConnectedCellModel *cellModel = charList[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKRGNormalConnectedCell *cell = [MKRGNormalConnectedCell initCellWithTableView:tableView];
    NSDictionary *dic = self.sectionList[indexPath.section];
    NSArray *charList = dic[@"charList"];
...
#pragma mark - MKRGNormalConnectedCellDelegate
- (void)rg_normalConnectedCell_writeButtonPressed:(NSInteger)section
    ...
    @weakify(self);
    [self.writeAlert showAlertWithValue:@"" dismissNote:@"mk_rg_needDismissAlert" cancelAction:^{
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKRGReceiveGatewayDisconnectDeviceNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKRGReceiveGatewayConnectedDeviceDatasNotification
                                               object:nil];
}
...
- (void)parseTableDatas {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        for (NSInteger j = 0; j < charCount; j ++) {
            ...
            MKRGNormalConnectedCellModel *cellModel = [[MKRGNormalConnectedCellModel alloc] init];
            cellModel.section = i;
            cellModel.row = j;
...
- (void)writeData:(NSString *)data serviceUUID:(NSString *)serverUUID characteristic:(NSString *)characteristic {
    ...
    [MKRGMQTTInterface rg_writeValueToDeviceWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] value:data serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)readDataWithServiceUUID:(NSString *)serverUUID
    ...
    [MKRGMQTTInterface rg_readCharacteristicValueWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
characteristic:(NSString *)characteristic
    ...
    [MKRGMQTTInterface rg_notifyCharacteristic:notify bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (MKRGConnectedDeviceWriteAlertView *)writeAlert {
    if (!_writeAlert) {
        _writeAlert = [[MKRGConnectedDeviceWriteAlertView alloc] init];
    }
    return _writeAlert;
}
