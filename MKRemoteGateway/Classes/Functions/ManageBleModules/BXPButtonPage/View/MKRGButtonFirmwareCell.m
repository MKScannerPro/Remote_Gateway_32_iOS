...
@implementation MKRGButtonFirmwareCellModel
@end

@interface MKRGButtonFirmwareCell ()
...
@implementation MKRGButtonFirmwareCell

+ (MKRGButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKRGButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKRGButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKRGButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKRGButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(rg_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate rg_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKRGButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKRGButtonFirmwareCellModel.class]) {
        return;
    }
