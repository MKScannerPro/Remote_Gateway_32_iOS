...
@interface MKRGMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKRGMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKRGMqttServerModel *dataModel;

@property (nonatomic, strong)MKRGMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKRGMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKRGMqttServerController

- (void)dealloc {
    NSLog(@"MKRGMqttServerController销毁");
}
...
#pragma mark - MKRGMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)rg_selectedServerParams:(NSString *)fileName {
    [MKRGExcelDataManager parseDeviceExcel:fileName
        ...
        MKRGMqttServerModel *model = [MKRGMqttServerModel mk_modelWithJSON:returnData];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMqttServerController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKRGMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKRGMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKRGMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKRGMqttServerConfigFooterViewModel alloc] init];
    }
