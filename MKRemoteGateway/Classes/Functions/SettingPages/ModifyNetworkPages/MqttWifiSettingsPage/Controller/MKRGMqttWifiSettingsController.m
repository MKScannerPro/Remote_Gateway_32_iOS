//
//  MKRGMqttWifiSettingsController.m
...
@interface MKRGMqttWifiSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKRGMqttWifiSettingsModel *dataModel;

@end

@implementation MKRGMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKRGMqttWifiSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteGateway", @"MKRGMqttWifiSettingsController", @"rg_saveIcon.png") forState:UIControlStateNormal];
...
- (MKRGMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKRGMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
