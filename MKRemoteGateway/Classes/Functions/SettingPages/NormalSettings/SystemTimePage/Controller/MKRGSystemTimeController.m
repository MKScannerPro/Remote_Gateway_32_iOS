...
@interface MKRGSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKRGSystemTimeCellDelegate,
...
@implementation MKRGSystemTimeController

- (void)dealloc {
    NSLog(@"MKRGSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKRGNTPServerController *vc = [[MKRGNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKRGSystemTimeCell *cell = [MKRGSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKRGSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKRGMQTTInterface rg_readDeviceUTCTimeWithMacAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKRGMQTTInterface rg_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKRGSystemTimeCellModel *cellModel = [[MKRGSystemTimeCellModel alloc] init];
    cellModel.index = 0;
