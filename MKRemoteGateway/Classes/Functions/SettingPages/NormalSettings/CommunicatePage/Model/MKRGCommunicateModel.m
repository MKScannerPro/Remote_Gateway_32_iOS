...
@interface MKRGCommunicateModel ()
...
@implementation MKRGCommunicateModel
...
- (BOOL)readCommunicateTimeout {
    ...
    [MKRGMQTTInterface rg_readCommunicateTimeoutWithMacAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configCommunicateTimeout {
    __block BOOL success = NO;
    [MKRGMQTTInterface rg_configCommunicationTimeout:[self.timeout integerValue] macAddress:[MKRGDeviceModeManager shared].macAddress topic:[MKRGDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
